...
$select=$conn->prepare("SELECT * FROM shafaye1_weblog_ketab_sara.writers WHERE writers.id=?") ;
$select->bindValue(1,$_GET['id']) ;
$select->execute() ;
$writers=$select->fetchAll(PDO::FETCH_ASSOC) ;
//var_dump($writers) ;	// namayeshe etelaat roye safhe sayt baraye Teste didane khodemun baraye 
...
if(isset($_POST['submit']))
{
	...
	$insert=$conn->prepare("UPDATE shafaye1_weblog_ketab_sara.writers SET username=? , image=? , bio=? WHERE id=?") ;
...
<div class="add-edit-blog">
	...
	<fieldset>
		<legend> افزودن نویسنده جدید </legend>
		...
		<input type="text" class="form-control" name="username" id="username" placeholder="موضوع مقاله" value="<?= $writer['username']; ?>">
		<div id="msg_username"> لطفا موضوع مقاله را وارد کنید </div>
	...
	<textarea name="editor1" id="editor1" placeholder="توضیحات"></textarea>
		...
		<input type="text" name="image" class="form-control" id="image" placeholder="لینک عکس" value="<?= $writer['image']; ?>">
